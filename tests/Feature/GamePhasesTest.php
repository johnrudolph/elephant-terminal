...
it('ends the game when a player has a 2x2 grid anywhere', function () {
    ...
    expect($this->game->state()->victors)->toBe([$this->player_1->id]);
});
...
it('can have multiple winners', function () {
    ...
    expect($this->game->state()->status)->toBe('complete');
    expect(collect($this->game->state()->victors))->toContain($this->player_1->id);
    expect(collect($this->game->state()->victors))->toContain($this->player_2->id);
});
